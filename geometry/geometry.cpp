...
const geometry::Vec3d verts[146] = {
...
void computePixelCoordinates(
    const geometry::Vec3d pWorld,
    geometry::Vec2i &pRaster,
    const geometry::Matrix44d &worldToCamera,
...
{
    geometry::Vec3d pCamera (pWorld * worldToCamera);
    geometry::Vec2d pScreen;
...
{
//    geometry::Vec3d v(0, 1, 2);
//    std::cout << v << std::endl;
//    geometry::Matrix44d a, b, c;
//    c = a * b;
//
//    geometry::Matrix44d d(0.707107, 0, -0.707107, 0, -0.331295, 0.883452, -0.331295, 0, 0.624695, 0.468521, 0.624695, 0, 4.000574, 3.00043, 4.000574, 1);
//    std::cout << d << std::endl;
//    d.invert();
//    std::cout << d << std::endl;
//
//    geometry::Matrix44d m(0.718762, 0.615033, -0.324214, 0, -0.393732, 0.744416, 0.539277, 0, 0.573024, -0.259959, 0.777216, 0, 0.526967, 1.254234, -2.53215, 1);
//    geometry::Vec3d Plocal(-0.5, 0.5, -0.5);
//    geometry::Vec3d Pworld( Plocal * m);
//    std::cout << Pworld << std::endl;
//
//    m.invert();
//    geometry::Vec3d Plocal2(Pworld * m);
//    std::cout << Plocal2 << std::endl;

    std::ofstream out ("./tree.svg");
    out << "<svg version=\"1.1\" xmlns:xlink=\"http://www.w3.org/1999/xlink\" xmlns=\"http://www.w3.org/2000/svg\" height=\"512\" width=\"512\">" << std::endl;
    geometry::Matrix44d cameraToWorld(0.871214, 0, -0.490904, 0, -0.192902, 0.919559, -0.342346, 0, 0.451415, 0.392953, 0.801132, 0, 14.777467, 29.361945, 27.993464, 1);
    geometry::Matrix44d worldToCamera (cameraToWorld.inverse());
    std::cout << worldToCamera << std::endl;
    double canvasWidth = 2, canvasHeight = 2;
    ...
    for (uint32_t i = 0; i < numTris; ++i) {
        const geometry::Vec3d &v0World = verts[tris[i * 3]];
        const geometry::Vec3d &v1World = verts[tris[i * 3 + 1]];
        const geometry::Vec3d &v2World = verts[tris[i * 3 + 2]];
        geometry::Vec2i v0Raster, v1Raster, v2Raster;
        ...
        std::cout << v0Raster << ", " << v1Raster << ", " << v2Raster << std::endl;
        out << "<line x1=\"" << v0Raster.x << "\" y1=\"" << v0Raster.y << "\" x2=\"" << v1Raster.x << "\" y2=\"" << v1Raster.y << "\" style=\"stroke:rgb(0,0,0);stroke-width:1\" />" << std::endl;
        out << "<line x1=\"" << v1Raster.x << "\" y1=\"" << v1Raster.y << "\" x2=\"" << v2Raster.x << "\" y2=\"" << v2Raster.y << "\" style=\"stroke:rgb(0,0,0);stroke-width:1\" />" << std::endl;
        out << "<line x1=\"" << v2Raster.x << "\" y1=\"" << v2Raster.y << "\" x2=\"" << v0Raster.x << "\" y2=\"" << v0Raster.y << "\" style=\"stroke:rgb(0,0,0);stroke-width:1\" />" << std::endl;
    }
    out << "</svg>" << std::endl;
